...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(32, 32, (5, 5), stride=1, padding=1)
        self.convnorm2 = nn.BatchNorm2d(32)

        self.pool1 = nn.MaxPool2d(kernel_size=(2, 2), stride=2)

        self.conv3 = nn.Conv2d(32, 64, (5, 5), stride=1, padding=1)
        self.convnorm3 = nn.BatchNorm2d(64)

        self.conv4 = nn.Conv2d(64, 64, (5, 5), stride=1, padding=1)
        self.convnorm4 = nn.BatchNorm2d(64)

        self.pool2 = nn.MaxPool2d(kernel_size=(2, 2), stride=2)

        self.linear1 = nn.Linear(179776, 1024)
        self.linear1_bn = nn.BatchNorm1d(1024)
        self.linear2 = nn.Linear(1024, 256)
        self.linear2_bn = nn.BatchNorm1d(256)
        self.linear3 = nn.Linear(256, 149)

    def forward(self, x):
        x = self.convnorm1(self.relu(self.conv1(x)))
        x = self.pool1(self.convnorm2(self.relu(self.conv2(x))))
        x = self.convnorm3(self.relu(self.conv3(x)))
        x = self.pool2(self.convnorm4(self.relu(self.conv4(x))))
        x = self.linear1_bn(self.relu(self.linear1(x.view(len(x), -1))))
        x = self.linear2_bn(self.relu(self.linear2(x)))
        x = self.linear3(x)
        return x
